...
import { ToolResponse, FoodOrderParams } from '../types';
...
export class FoodOrderingTool {
  private ondcService: ONDCService;
  ...
  async handler(args: unknown): Promise<ToolResponse> {
    ...
    try {
      // Validate input
      const params = FoodOrderSchema.parse(args) as FoodOrderParams;
      ...
      if (params.dishType && params.dishType !== 'both') {
        filteredRestaurants = restaurants.filter(restaurant =>
          restaurant.menu?.some(item => 
            params.dishType === 'veg' ? item.isVeg : !item.isVeg
          )
        );
      }
