...
export class NammaYatriService {
  ...
  async bookRide(estimateId: string, customerDetails: {
    name: string;
    phone: string;
  }): Promise<any> {
    try {
  ...
  async getRideStatus(bookingId: string): Promise<any> {
    try {
      const statusPayload = {
        context: {
          domain: 'ONDC:TRV11',
  ...
  private parseRideEstimates(data: any): RideEstimate[] {
    try {
  ...
  private parseBookingResponse(data: any) {
    return {
  ...
  private parseStatusResponse(data: any) {
    return {
      status: data.message?.order?.status,
  ...
  private getMockRideEstimates(params: { pickup: string; destination: string; rideType?: string }): RideEstimate[] {
  ...
  private calculateMockDistance(pickup: string, destination: string): number {
